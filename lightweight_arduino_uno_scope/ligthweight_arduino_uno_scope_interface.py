...
MAX_SAMPLES_NUMBER = 2000

class MainWindow(wx.Frame):
    title = 'Ligthweight Arduino Uno Scope'
    def __init__(self):
        #dpi of the plot
        self.dpi = 100
        ...
        self.timediv_ch1 = wx.Choice(self.panel, -1,pos = (ch1_timediv_x + 20, ch1_timediv_y + 20), size = (60, 60),
                                    choices = ["13us","39us","65us","130us","325us","650us","1ms","1.4ms","1.95ms","2.6ms","3.9ms","4.16ms","5.2ms","5.7ms","7.8ms","10.4ms","15.6ms","20.8ms"])
            ...
            if(self.ch1_paused == False):
                ...
                if (self.ch1_trigger == False):
                    if (self.request == True):
                        ...
                        #frequncy dependency
                        if(self.timediv_ch1.GetSelection() <= 9):
                            self.ser.write("\x01")
                        elif(self.timediv_ch1.GetSelection() == 10 or self.timediv_ch1.GetSelection() == 12):
                            self.ser.write("\x02")
                        elif(self.timediv_ch1.GetSelection() == 11):
                            self.ser.write("\x03")
                        elif(self.timediv_ch1.GetSelection() >= 13 and self.timediv_ch1.GetSelection() <= 15):    
                            self.ser.write("\x03")
                        elif(self.timediv_ch1.GetSelection() > 15):
                            self.ser.write("\x04")
                        self.ser.write("\x00")
                        self.ser.write("\x00")
                        self.ser.write("\x00")
    ...
    def time_div_adjust(self,event):
        ...
        if (self.timediv_ch1.GetSelection() == 0):
            sample_num = int(MAX_SAMPLES_NUMBER*0.005)
        elif (self.timediv_ch1.GetSelection() == 1):
            sample_num = int(MAX_SAMPLES_NUMBER*0.015)
        elif (self.timediv_ch1.GetSelection() == 2):
            sample_num = int(MAX_SAMPLES_NUMBER*0.025)
        elif (self.timediv_ch1.GetSelection() == 3):
            sample_num = int(MAX_SAMPLES_NUMBER*0.05)
        elif (self.timediv_ch1.GetSelection() == 4):
            sample_num = int(MAX_SAMPLES_NUMBER*0.125)
        elif (self.timediv_ch1.GetSelection() == 5):
            sample_num = int(MAX_SAMPLES_NUMBER*0.25)
        elif (self.timediv_ch1.GetSelection() == 6 or self.timediv_ch1.GetSelection() == 11):        
            sample_num = int(MAX_SAMPLES_NUMBER*0.4)
        elif (self.timediv_ch1.GetSelection() == 7 or self.timediv_ch1.GetSelection() == 13):
            sample_num = int(MAX_SAMPLES_NUMBER*0.5) 
        elif (self.timediv_ch1.GetSelection() == 8 or self.timediv_ch1.GetSelection() == 10 or self.timediv_ch1.GetSelection() == 14 or self.timediv_ch1.GetSelection() == 16):
            sample_num = int(MAX_SAMPLES_NUMBER*0.75) 
        elif (self.timediv_ch1.GetSelection() == 9 or self.timediv_ch1.GetSelection() == 12 or self.timediv_ch1.GetSelection() == 15 or self.timediv_ch1.GetSelection() == 17):
           sample_num = MAX_SAMPLES_NUMBER
